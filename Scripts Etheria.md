-- ============================================================================
-- ETHERIA GLOBALS - SCRIPT DE CREACIÓN DE TABLAS (POSTGRESQL)
-- ============================================================================
...
CREATE TABLE ExchangeRates (
    ...
    currencyId1 INTEGER REFERENCES Currencies(currencyId),
    currencyId2 INTEGER REFERENCES Currencies(currencyId),
...
CREATE TABLE CountriesRegulations (
...
CREATE TABLE IncotermsRules (
    ruleId SERIAL PRIMARY KEY,
    incotermId INTEGER REFERENCES Incoterms(incotermId),
    costTypeId INTEGER REFERENCES CostsTypes(costTypeId),
...
CREATE TABLE Products (
    ...
    categoryId INTEGER REFERENCES ProductsCategories(categoryId),
    unitOfMeasureId INTEGER REFERENCES UnitsOfMeasure(unitOfMeasureId),
...
CREATE TABLE Suppliers (
    ...
    countryId INTEGER REFERENCES Countries(countryId),
    addressId INTEGER REFERENCES Addresses(addressId),
    defaultCurrencyId INTEGER REFERENCES Currencies(currencyId),
...
CREATE TABLE ServicesProviders (
    ...
    countryId INTEGER REFERENCES Countries(countryId),
    addressId INTEGER REFERENCES Addresses(addressId),
    logisticsRoleId INTEGER REFERENCES LogisticsRoles(logisticsRoleId),
    transportMethodId INTEGER REFERENCES TransportMethods(transportMethodId),
...
CREATE TABLE Contacts (
    ...
    employeeId INTEGER REFERENCES Employees(employeeId) NULL,
    providerId INTEGER REFERENCES ServicesProviders(providerId) NULL,
    supplierId INTEGER REFERENCES Suppliers(supplierId) NULL,
    contactTypeId INTEGER REFERENCES ContactTypes(contactTypeId),
...
CREATE TABLE ExchangeHistories (
    ...
    updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updatedBy INTEGER,
    exchangeRateId INTEGER REFERENCES ExchangeRates(exchangeRateId)
);
...
CREATE TABLE PurchaseOrders (
    ...
    incotermId INTEGER REFERENCES Incoterms(incotermId),
    destinationAddressId INTEGER REFERENCES Addresses(addressId),
    supplierCurrencyId INTEGER REFERENCES Currencies(currencyId),
    etheriaCurrencyId INTEGER REFERENCES Currencies(currencyId),
    currentStatusId INTEGER REFERENCES StatusTypes(StatusTypeId),
    exchangeRate DECIMAL(18, 6),
...
CREATE TABLE PurchaseOrdersLandedCosts (
    purchaseCostsId SERIAL PRIMARY KEY,
    purchaseOrderId INTEGER REFERENCES PurchaseOrders(purchaseOrderId),
    providerId INTEGER REFERENCES ServicesProviders(providerId),
    costTypeId INTEGER REFERENCES CostsTypes(costTypeId),
    originalAmount NUMERIC(10, 2),
    exchangeRate DECIMAL(18, 6),
    totalAmountUSD NUMERIC(10, 2),
    currencyId INTEGER REFERENCES Currencies(currencyId),
...
CREATE TABLE PurchaseOrdersTracking (
    orderTrackingId SERIAL PRIMARY KEY,
    purchaseOrderId INTEGER REFERENCES PurchaseOrders(purchaseOrderId),
    statusTypeId INTEGER REFERENCES StatusTypes(StatusTypeId),
    currentAddressId INTEGER REFERENCES Addresses(addressId),
...
-- ============================================================================
-- ETHERIA GLOBALS - ÍNDICES RECOMENDADOS (PostgreSQL)
-- ============================================================================
...
CREATE INDEX idx_providers_country ON ServicesProviders(countryId);
CREATE INDEX idx_providers_role ON ServicesProviders(logisticsRoleId);

-- Catálogo de Productos
CREATE INDEX idx_products_category ON Products(categoryId);
CREATE INDEX idx_products_measure ON Products(unitOfMeasureId);

-- Finanzas y Transacciones
CREATE INDEX idx_exchange_curr1 ON ExchangeRates(currencyId1);
CREATE INDEX idx_exchange_curr2 ON ExchangeRates(currencyId2);

CREATE INDEX idx_po_supplier ON PurchaseOrders(supplierId);
CREATE INDEX idx_po_status ON PurchaseOrders(currentStatusId);
CREATE INDEX idx_po_incoterm ON PurchaseOrders(incotermId);
CREATE INDEX idx_po_dest_address ON PurchaseOrders(destinationAddressId);

CREATE INDEX idx_podetails_po ON PurchaseOrdersDetails(purchaseOrderId);
CREATE INDEX idx_podetails_product ON PurchaseOrdersDetails(productId);

CREATE INDEX idx_potracking_po ON PurchaseOrdersTracking(purchaseOrderId);
CREATE INDEX idx_potracking_status ON PurchaseOrdersTracking(statusTypeId);

-- ============================================================================
-- ETHERIA GLOBALS - FULL DATABASE POPULATION SCRIPT (MIN 5 RECORDS, 0 NULLS, RANDOM DATES)
-- ============================================================================
...
BEGIN
    ...
    TRUNCATE TABLE 
        Countries, Currencies, LogisticsRoles, TransportMethods, Incoterms, 
        StatusTypes, ProductsCategories, UnitsOfMeasure, PaymentMethods, 
        CostsTypes, ContactTypes, States, ExchangeRates, CountriesRegulations, 
        IncotermsRules, Products, Cities, Addresses, Employees, Suppliers, 
        ServicesProviders, Contacts, ExchangeHistories, PurchaseOrders, 
        PurchaseOrdersDetails, PurchaseOrdersLandedCosts, PurchaseOrdersTracking 
    RESTART IDENTITY CASCADE;

    -- ==========================================
    -- 1. CATÁLOGOS BASE (Mínimo 5 registros)
    -- ==========================================
    INSERT INTO Countries (isoCode, countryName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO Currencies (currencySymbol, currencyName, isActive, postTime, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO LogisticsRoles (roleName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO TransportMethods (transportName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO Incoterms (incotermCode, description, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO StatusTypes (statusName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO ProductsCategories (categoryName, description, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO UnitsOfMeasure (symbol, measureName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO PaymentMethods (paymentMethodName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO CostsTypes (costTypeName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO ContactTypes (contactTypeName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    -- ==========================================
    INSERT INTO States (countryId, stateName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO Cities (stateId, cityName, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO Addresses (cityId, address, zipCode, latitude, longitude, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    -- ==========================================
    INSERT INTO ExchangeRates (currencyId1, currencyId2, exchangeRate, postTime, checkSum, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO CountriesRegulations (countryId, regulationName, documentURL, expiryDate, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO IncotermsRules (incotermId, costTypeId, isImporterResponsibility, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    -- ==========================================
    INSERT INTO Employees (addressId, firstName, lastName, isActive, hiringDate, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO Suppliers (countryId, addressId, defaultCurrencyId, supplierName, fdaRegistrationNumber, taxIdentificationNumber, isActive, isGmpCertified, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO ServicesProviders (countryId, addressId, logisticsRoleId, transportMethodId, providerName, trackingPortalUrl, taxIdentificationNumber, specificAttributes, liabilityInsuranceLimit, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    INSERT INTO Contacts (employeeId, providerId, supplierId, contactTypeId, value, isActive, createdAt, updatedAt, updatedBy) VALUES 
    ...
    -- ==========================================
    INSERT INTO Products (categoryId, unitOfMeasureId, productName, description, isActive, createdAt, updatedAt, updatedBy)
    SELECT 
        (random() * 4 + 1)::int, 
        (random() * 4 + 1)::int, 
        'Etheria Item ' || gs, 
    ...
    -- ==========================================
    INSERT INTO ExchangeHistories (startDateTime, endDateTime, currencyId1, currencyId2, exchangeRate, postTime, checkSum, createdAt, updatedAt, updatedBy, exchangeRateId) VALUES 
    ...
    FOR v_po_id IN SELECT purchaseOrderId FROM PurchaseOrders LOOP
        ...
        INSERT INTO PurchaseOrdersTracking (purchaseOrderId, statusTypeId, currentAddressId, eventDate, auditInfo, isActive, createdAt, updatedAt, updatedBy) VALUES 
        ...
        INSERT INTO PurchaseOrdersLandedCosts (purchaseOrderId, providerId, costTypeId, originalAmount, exchangeRate, totalAmountUSD, currencyId, description, isActive, createdAt, updatedAt, createdBy, updatedBy) VALUES 
    ...
    RAISE NOTICE '¡Éxito! Las 27 tablas están pobladas con mínimo 5 registros, 0 valores NULL y fechas orgánicas.';
END $$;